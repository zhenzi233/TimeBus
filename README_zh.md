...
- **潜行 + 右键方块**：加速一次，消耗杖内 **10 mB 时间流体 + 1000 AE**。速度取决于通过 AE2 单元工作台（Cell Workbench）插入的速度卡，倍率走独立的 `wandSpeedMultipliers` 配置。
  - **对 MM（Modular Machinery）控制器**（需开启 `MM Acceleration Enabled`）：点击后**当前正在运行的配方以倍率加速直到完成**——配方时长缩短为 1/倍率，每 tick 耗电/产出按 `MM Energy Cost Follows Speed` 配置相应放大（单次配方总耗电守恒）；配方完成（或失败）后自动恢复原速，不留下任何持久状态；机器空闲时点击不消耗资源并提示。
